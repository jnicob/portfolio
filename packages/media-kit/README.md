...
| `onPositionChange` | `(position: number) => void` | `undefined`    | Called with the new position (`0`–`100`) on every update. **Pointer drag reports fractional values** (derived from pixel geometry); keyboard steps (arrows, Home/End, Page Up/Down) always report integers. |
